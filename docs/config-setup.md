...
2. Add variable placeholders using `VARIABLENAME` format
3. Update processing in `modules/config.sh`
4. Add validation rules as needed

### Environment Variables for Hardware Control

For automated or scripted installations, use these environment variables:

```bash
# Force NVIDIA configuration
FORCE_NVIDIA=true ./install.sh quick

# Force non-NVIDIA configuration (Intel/AMD)
FORCE_NO_NVIDIA=true ./install.sh quick

# Use generic minimal configuration
FORCE_GENERIC=true ./install.sh quick

# Example: Laptop installation without NVIDIA
FORCE_NO_NVIDIA=true ./install.sh full

# Example: Desktop with RTX 4090
FORCE_NVIDIA=true ./install.sh full
```

### Extending Hardware Detection

1. Add detection logic in `detect_and_configure_nvidia()`
2. Create configuration blocks as needed
